...
if __name__ == '__main__':
    ...
    # Delete any existing webhook first (important!) - using sync method
    try:
        import requests
        resp = requests.get(f"https://api.telegram.org/bot{BOT_TOKEN}/deleteWebhook?drop_pending_updates=true", timeout=10)
        print(f"Webhook delete response: {resp.status_code} - {resp.text}")
    except Exception as e:
