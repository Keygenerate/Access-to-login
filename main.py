...
def init_database():
    ...
    c.execute("""
        CREATE TABLE IF NOT EXISTS users (
            ...
            chat_id BIGINT, joined_at TIMESTAMP, is_blocked INTEGER DEFAULT 0, is_banned INTEGER DEFAULT 0
        )
    """)
    c.execute("""
        CREATE TABLE IF NOT EXISTS tokens (
            id SERIAL PRIMARY KEY, user_id BIGINT, open_id TEXT,
            access_token TEXT UNIQUE, captured_at TIMESTAMP,
            FOREIGN KEY(user_id) REFERENCES users(user_id)
        )
    """)
    c.execute("""
        CREATE TABLE IF NOT EXISTS broadcasts (
            id SERIAL PRIMARY KEY, message TEXT, sent_at TIMESTAMP, total_sent INTEGER
        )
    """)
    c.execute("CREATE TABLE IF NOT EXISTS config (key TEXT PRIMARY KEY, value TEXT)")
    c.execute("""
        CREATE TABLE IF NOT EXISTS sessions_log (
            id SERIAL PRIMARY KEY, user_id BIGINT, access_token TEXT, open_id TEXT,
            started_at TIMESTAMP, stopped_at TIMESTAMP, is_active INTEGER DEFAULT 1
        )
    """)
...
def db_add_user(user_id, username, first_name, chat_id):
    try:
        conn = get_db()
        c = conn.cursor()
        c.execute("INSERT INTO users (user_id, username, first_name, chat_id, joined_at) VALUES (%s, %s, %s, %s, %s) ON CONFLICT (user_id) DO NOTHING",
...
def db_save_token(user_id, open_id, access_token):
    try:
        conn = get_db(); c = conn.cursor()
        c.execute("INSERT INTO tokens (user_id, open_id, access_token, captured_at) VALUES (%s, %s, %s, %s) ON CONFLICT (access_token) DO NOTHING",
...
def db_ban_user(user_id, ban=True):
    try:
        conn = get_db(); c = conn.cursor()
        c.execute("UPDATE users SET is_banned = %s WHERE user_id = %s", (1 if ban else 0, user_id))
        conn.commit(); conn.close()
        return True
    except Exception as e:
...
def db_save_broadcast(message, total_sent):
    try:
        conn = get_db(); c = conn.cursor()
        c.execute("INSERT INTO broadcasts (message, sent_at, total_sent) VALUES (%s, %s, %s)", (message, datetime.now(), total_sent))
        conn.commit(); conn.close()
    except Exception as e:
...
def db_set_maintenance(status):
    try:
        conn = get_db(); c = conn.cursor()
        c.execute("INSERT INTO config (key, value) VALUES ('maintenance', %s) ON CONFLICT (key) DO UPDATE SET value = %s",
...
def db_log_session(user_id, access_token, open_id, started_at, stopped_at=None, is_active=1):
    try:
        conn = get_db(); c = conn.cursor()
        c.execute("INSERT INTO sessions_log (user_id, access_token, open_id, started_at, stopped_at, is_active) VALUES (%s, %s, %s, %s, %s, %s)",
...
def db_stop_active_sessions(user_id):
    try:
        conn = get_db(); c = conn.cursor()
        c.execute("UPDATE sessions_log SET stopped_at = %s, is_active = 0 WHERE user_id = %s AND is_active = 1", (datetime.now(), user_id))
        conn.commit(); conn.close()
    except Exception as e:
...
def db_get_active_session(user_id):
    try:
        conn = get_db(); c = conn.cursor()
        c.execute("SELECT * FROM sessions_log WHERE user_id = %s AND is_active = 1 ORDER BY id DESC LIMIT 1", (user_id,))
        result = c.fetchone(); conn.close()
        return result
    except Exception as e:
...
def db_get_user_session_count(user_id):
    try:
        conn = get_db(); c = conn.cursor()
        c.execute("SELECT COUNT(*) FROM sessions_log WHERE user_id = %s", (user_id,))
        result = c.fetchone()[0]; conn.close()
        return result
    except Exception as e:
