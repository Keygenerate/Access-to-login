...
async def handle_admin(update, context):
    ...
    if context.user_data.get("awaiting_ban"):
        ...
        try:
            ...
            await update.message.reply_text(f"Invalid ID: {e}")
        context.user_data["awaiting_ban"] = False
        await update.message.reply_html("Done!", reply_markup=get_keyboard(uid))
        return
...
# ═══════════════════════════════════════════════════════════════
# FLASK ROUTES (Webhook + Health Check + Proxy)
# ═══════════════════════════════════════════════════════════════
@app.route('/')
def home():
...
def webhook():
    ...
    if request.method == 'POST':
        try:
            ...
            print(f"Webhook received: {json_data.get('update_id', 'N/A')}")
            update = Update.de_json(json_data, bot_app.bot)

            # CRITICAL FIX: Use process_update for v21
...
# ═══════════════════════════════════════════════════════════════
# MAIN - WEBHOOK MODE (100% works on Render free tier)
# ═══════════════════════════════════════════════════════════════
if __name__ == '__main__':
    ...
    if webhook_url:
        ...
        try:
            ...
            print(f"Webhook set response: {resp.status_code} - {resp.text}")
        except Exception as e:
            print(f"Webhook setup error: {e}")

        # Start Flask (this is the ONLY thing running - no polling!)
        print(f"Starting Flask server on port {PORT}...")
        app.run(host='0.0.0.0', port=PORT, debug=False, threaded=True)
    else:
        # Local development fallback
        print("No RENDER_EXTERNAL_URL found, falling back to polling...")
